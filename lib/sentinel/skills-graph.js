...
function getSkillStats(skillId) {
  ...
  const averageEffectiveness = nodes.length
    ? Number(
        (nodes.reduce((acc, node) => acc + (node.effectiveness || 0), 0) / nodes.length).toFixed(4),
      )
    : 0;
...
function suggestSkillsForTask(taskDescription, options = {}) {
  ...
  for (const node of Object.values(graph.skills)) {
    ...
    const score =
      tokenMatches * 3 + taskTypeMatches * 2 + node.effectiveness * 2 + Math.log1p(node.used);
    if (score <= 0) continue;

    const reasonParts = [];
    if (tokenMatches > 0)
      reasonParts.push(`${tokenMatches} keyword match${tokenMatches > 1 ? "es" : ""}`);
    if (taskTypeMatches > 0) reasonParts.push(`used for similar task types (${taskTypeMatches})`);
    if (node.effectiveness > 0)
      reasonParts.push(`effectiveness ${(node.effectiveness * 100).toFixed(0)}%`);
  ...
  return suggestions.sort((a, b) => b.score - a.score).slice(0, Math.max(0, limit));
}
...
function buildToolsAutoSection(skills, stats) {
  ...
  if (topUsed.length > 0) {
    ...
    for (const entry of topUsed) {
      lines.push(
        `- ${entry.name} (\`${entry.skillId}\`) - ${entry.used} uses, effectiveness ${(entry.effectiveness * 100).toFixed(0)}%`,
      );
    }
    lines.push("");
  }
...
function syncWithToolsMd() {
  ...
  const hasAutoSection =
    toolsContent.includes(AUTO_SECTION_START) && toolsContent.includes(AUTO_SECTION_END);
  if (hasAutoSection) {
...
// Track skill dependencies and usage patterns
const skillGraph = new Map();

function registerSkillUse(skillId, context) {
  if (!skillGraph.has(skillId)) {
    skillGraph.set(skillId, { uses: 0, contexts: new Set(), lastUsed: null });
  }
  const node = skillGraph.get(skillId);
  node.uses++;
...
function getSkillStats(skillId) {
  return skillGraph.get(skillId) || null;
}

export {  registerSkillUse, getSkillStats, skillGraph  };
