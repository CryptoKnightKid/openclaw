...
const NAMESPACES = {
  ...
  session: {
    ...
    file: "session.md", // Store in file, not directory root
    ttl: "1h",
    weight: 0.3,
  },
  decisions: {
...
function store(namespace, data) {
  ...
  if (namespace === "decisions") {
    ...
    // Session uses a file inside the directory
    const file = path.join(config.path, config.file || "session.md");
    appendMarkdown(file, entry);
  } else {
    // Core uses markdown directly
...
function calculateScore(memory, query, weights) {
  const text =
    `${memory.title || ""} ${memory.content || ""} ${memory.decision || ""}`.toLowerCase();
...
function cleanupExpired() {
  for (const [ns, config] of Object.entries(NAMESPACES)) {
    ...
    // Handle experiences namespace (directory of files)
    if (ns === "experiences") {
    ...
    // Handle session and decisions namespaces (single JSONL files)
    if (ns === "session" || ns === "decisions") {
      if (!fs.existsSync(config.path)) continue;
      
      // Read all entries
      const entries = loadJsonl(config.path);
